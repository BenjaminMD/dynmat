...
def get_atomic_mass(atom_label: str) -> float:
    ...
    return float(element(atom_label).atomic_weight)  # type: ignore


@cache
def calculate_force_constant(mass1: float, mass2: float) -> float:
    """
    Calculate force constant from masses.
    Order of masses doesn't matter.
    """
...
def get_force(
    ...
    atom1_mass = get_atomic_mass(atom1_label)
    atom2_mass = get_atomic_mass(atom2_label)
    k = calculate_force_constant(atom1_mass, atom2_mass)
    ...
    # Calculate force
    # print( equi)
    return (k * (equi - np.array(new_dist))) * direction
